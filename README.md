...
- 🔸 **[llm-etl-pipeline-template](https://github.com/HamedDaabies/llm-etl-pipeline-template)** — OpenAI ETL scaffold with structured outputs, retries, and deterministic enrichment — `temperature=0`, source-pinned ids, canonicalized tags — so re-runs upsert instead of orphaning rows; per-model cost tracking with pre-flight budget estimation and a hard cap that halts enrichment; typed Pydantic config and pluggable JSONL/SQLite loaders that close cleanly on error
