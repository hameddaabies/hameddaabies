<h1 align="center">Hi, I'm Hamed Daabies</h1>

<p align="center">
  <b>Data Engineer</b> · LLM Pipelines · Web Scraping · ETL · Python · AWS / GCP
</p>
...
I build **production-grade data pipelines** that scrape, enrich with LLMs, and deliver clean structured output at scale. Based in Alexandria, Egypt — working remotely with teams in **Canada 🇨🇦 and the Netherlands 🇳🇱**.

- 🏭 **Currently** — engineering an AI-powered product data platform for a Canadian e-commerce company: OpenAI-integrated ETL, Scrapy + Playwright crawlers, embedding-based product matching on AWS
- 🛒 **Previously** — ML-based product matching + GCP ETL for a Dutch grocery comparison platform, deduplicating 100K+ records across 5 retailers with no standardized IDs
- 🏆 **Top Rated Plus on Upwork** — 100% Job Success Score, 20+ projects delivered
- 🎓 **Microsoft Data Engineer** (DEPI, accredited by Egypt's Ministry of Communications & IT) + **Harvard CS50**
...
- **LLM integration** — OpenAI API, structured JSON outputs, deterministic enrichment, retry & cost tracking
...
- **Reproducible runs** — idempotent upserts, deterministic ranking tie-breaks, fixture-driven tests that guarantee identical results on re-run
...
![sentence-transformers](https://img.shields.io/badge/sentence--transformers-FF9D00?style=flat-square&logo=huggingface&logoColor=white)
...
- 🔸 **[llm-etl-pipeline-template](https://github.com/HamedDaabies/llm-etl-pipeline-template)** — OpenAI ETL scaffold with structured outputs, retries, and deterministic enrichment — `temperature=0`, source-pinned ids, canonicalized tags — so re-runs upsert instead of orphaning rows; per-model cost tracking with pre-flight budget estimation and a hard cap that halts enrichment; typed Pydantic config and pluggable JSONL/SQLite loaders that close cleanly on error; a row that fails enrichment is logged and skipped so one bad record can't abort the rest of the batch, while a tripped cost cap still halts the run
- 🔸 **[scrapy-playwright-template](https://github.com/HamedDaabies/scrapy-playwright-template)** — Scrapy + Playwright scaffold with UA rotation, proxy + cookie-session + URL-deny middlewares and a Pydantic validation pipeline that records schema-drop crawl stats; fixture-tested example spiders cover both plain HTTP and a browser-rendered page whose parse is gated on `PageMethod("wait_for_selector", ...)`, with image/font/media subresources aborted so headless renders skip assets the parser never reads
